...
export const fetchWinners = createAsyncThunk(
  ...
  async ({ page, limit, sort, order }: { page: number, limit: number, sort: Sort, order: Order }) => {
    ...
    const items: WinnerWithCarInfo[] = winnersData.items.map((winner, index) => ({
      ...winner,
    ...
    return {
      items, totalCount: winnersData.totalCount
    };
  }
);
