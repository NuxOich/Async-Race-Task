...
## Server: https://github.com/mikhama/async-race-api#get-cars

## Deploy: https://async-race-task.vercel.app/

## Instruction for running:
1. Clone and run the mock server locally: `git clone https://github.com/mikhama/async-race-api.git && cd async-race-api && npm install && npm start`
2. Open the [deployed link](https://async-race-task.vercel.app/)
3. Your browser may show a permission prompt (e.g. "wants to access other apps and services on this device") — this happens because the HTTPS-deployed site connects to `http://127.0.0.1:3000`. Click **Allow**.
4. If the app doesn't load data, make sure the local server is running, then reload the page.
...
> **Note:** `eslint-config-airbnb` (and `eslint-config-airbnb-typescript`) could not be integrated due to a version conflict with the TypeScript version used in this project. Instead, a custom ESLint flat config was written to mirror the Airbnb style guide's core rules, combined with `eslint-config-prettier` for formatting and `typescript-eslint`'s recommended rules for type safety. `eslint-config-airbnb` wasn't updated for 5 years.
